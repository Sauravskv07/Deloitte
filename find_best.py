...
Varieties=['type0', 'type1', 'type2', 'type3', 'type4', 'type5', 'type6','type7', 'type8', 'type9', 'type10', 'type11', 'type12', 'type13','type14', 'type15', 'type16', 'type17', 'type18', 'type19','type20']
Grades=['item14', 'item15', 'item16', 'item17']
...
mapping_index= json.load(open("./data/utils/mapping_index.json"))

distance_matrix= np.load('./data/utils/distance_matrix.npy')

fuel_matrix= np.load('./data/utils/fuel_matrix.npy')

print("Utilities Loaded")

print("Calculating Tomorrow's Predictions")

for variety in Varieties:
	predictions[variety] = {}
	for grade in Grades:
		predictions[variety][grade] = {}
		for state in States:
			for market in Markets:	
				try:
					path="./data/csv/Commodity1_price_updated_('"+state+"', '"+market+"', '"+variety+"', '"+grade+"').csv"
					df=pd.read_csv(path)

					if(predictions[variety][grade].get(state)==None):
						predictions[variety][grade][state]={}
				except:
					continue
				...
				current_req=df.iloc[-1]['Arrival_old']
				predicted_req=(1-y[2]+y[0])*current_req

				predictions[variety][grade][state][market]=math.floor(predicted_req*1000)
...
while True:
    ...
    if(predictions.get(product["variety"])!= None and predictions[product["variety"]].get(product["grade"])!= None):
        
        if(mapping_index.get(product["state"])!=None and mapping_index[product["state"]].get(product["market"])!=None):
            
            src_index=mapping_index[product["state"]][product["market"]]
            
            for state in mapping_index.keys():
                for market in mapping_index[state].keys():
                    dest_index=mapping_index[state][market]
                    cost_list.append((state,market,((fuel_matrix[src_index]+fuel_matrix[dest_index])*0.5)*distance_matrix[src_index][dest_index]))

            cost_list.sort(key = lambda x: x[2])

            for state,market,cost in cost_list:
                if(state in predictions[product["variety"]][product["grade"]].keys() and market in predictions[product["variety"]][product["grade"]][state].keys()):
                    count+=1
                    if(predictions[product["variety"]][product["grade"]][state][market]>quantity):
                        result={"state":state, "market":market, "prediction": quantity}
                        predictions[product["variety"]][product["grade"]][state][market]-=quantity
                        results[str(count)]=json.dumps(result)
                        break
                    else:
                        if(predictions[product["variety"]][product["grade"]][state][market]==0):
                            count-=1
                            continue

                        result={"state":state, "market":market, "prediction": predictions[product["variety"]][product["grade"]][state][market]}
                        quantity-=predictions[product["variety"]][product["grade"]][state][market]
                        predictions[product["variety"]][product["grade"]][state][market]=0
                        results[str(count)]=json.dumps(result)

    #print(results)

    socket.send_unicode(json.dumps(results))
